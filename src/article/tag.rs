...
impl Tag {
	pub fn set_from_text(text: &str) -> Result<HashSet<Tag>, Exception> {
		lazy_static! {
...
mod test {
	...
	fn single_element_array__single_tag() {
		let tags_text = "[$TAG]";
		let expected_tags: HashSet<_> = vec!["$TAG"]
			.into_iter()
			.map(ToString::to_string)
			.map(|text| Tag { text })
			.collect();
	...
	fn multiple_elements_array__multiple_tags() {
		...
		let expected_tags: HashSet<_> = vec!["$TAG", "$TOG", "$TUG"]
			.into_iter()
			.map(ToString::to_string)
			.map(|text| Tag { text })
			.collect();
	...
	fn multiple_elements_array_with_just_spaces_tag__empty_tag_is_ignored() {
		...
		let expected_tags: HashSet<_> = vec!["$TAG", "$TUG"]
			.into_iter()
			.map(ToString::to_string)
			.map(|text| Tag { text })
			.collect();
